...
export function PaymentBalanceChecker() {
  ...
  useEffect(() => {
    // Initialize payment methods - ensure all three are available
    const defaultMethods: PaymentMethod[] = [
      { type: "mpesa", balance: 0, currency: "KES", enabled: false },
      { type: "card", balance: 0, currency: "KES", enabled: false },
      { type: "paypal", balance: 0, currency: "KES", enabled: false },
    ]

    if (user?.preferences?.paymentMethods && user.preferences.paymentMethods.length > 0) {
      // Merge user preferences with defaults
      const methods = defaultMethods.map((defaultMethod) => {
        const userMethod = user.preferences?.paymentMethods?.find((m) => m.type === defaultMethod.type)
        return userMethod
          ? {
              ...defaultMethod,
  ...
  const togglePaymentMethod = (type: "mpesa" | "card" | "paypal", enabled: boolean) => {
    ...
    if (user) {
      ...
      // Ensure all payment methods exist in preferences
      const existingMethods = user.preferences?.paymentMethods || []
      const updatedPaymentMethods = [
        { type: "mpesa" as const, enabled: false, balance: 0, currency: "KES" as const },
        { type: "card" as const, enabled: false, balance: 0, currency: "KES" as const },
        { type: "paypal" as const, enabled: false, balance: 0, currency: "KES" as const },
      ].map((defaultMethod) => {
        const existing = existingMethods.find((m) => m.type === defaultMethod.type)
        const current = updatedMethods.find((m) => m.type === defaultMethod.type)
        return {
          type: defaultMethod.type,
          enabled: current?.enabled ?? existing?.enabled ?? defaultMethod.enabled,
          balance: 0,
            ...
            {enabledMethods.length > 0 && (
              <div className="p-4 bg-muted/50 rounded-lg">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-lg font-bold text-green-600">KSh {totalBalance.toFixed(2)}</span>
                </div>
                {lastCheck && (
